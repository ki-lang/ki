...
static unsigned int
singletable_crc32c(unsigned int crc, const void *buf, size_t size)
{
	const unsigned char *p = buf;
...
static const unsigned int sctp_crc_tableil8_o32[256] =
{
...
static const unsigned int sctp_crc_tableil8_o40[256] =
{
...
static const unsigned int sctp_crc_tableil8_o48[256] =
{
...
static const unsigned int sctp_crc_tableil8_o56[256] =
{
...
static const unsigned int sctp_crc_tableil8_o64[256] =
{
...
static const unsigned int sctp_crc_tableil8_o72[256] =
{
...
static const unsigned int sctp_crc_tableil8_o80[256] =
{
...
static const unsigned int sctp_crc_tableil8_o88[256] =
{
...
static unsigned int
crc32c_sb8_64_bit(unsigned int crc,
    const unsigned char *p_buf,
    unsigned int length,
    unsigned int init_bytes)
{
	unsigned int li;
	unsigned int term1, term2;
...
#else
		term1 = sctp_crc_tableil8_o56[(*(const unsigned int *) p_buf) & 0x000000FF] ^
		    sctp_crc_tableil8_o48[((*(const unsigned int *) p_buf) >> 8) & 0x000000FF];

		term2 = (*(const unsigned int *) p_buf) >> 16;
		crc = crc ^
		    term1 ^
...
static unsigned int multitable_crc32c(unsigned int crc32c, const unsigned char *buffer, unsigned int length) {
    unsigned int to_even_word;

    if (length == 0) {
        return (crc32c);
    }
    to_even_word = (4 - (((intptr_t)buffer) & 0x3));
